...
\begin{mathpar}
    ...
    \inferrule*[right=Abs]{
        \Gamma \proves \PiT x : \tau . \sigma : s
        \and
        \Gamma, x : \tau \proves e : \sigma
    } { \Gamma \proves \lambda x : \tau . e : \PiT x : \tau . \sigma }

    \inferrule*[right=App]{
        \Gamma \proves f : \PiT x : \tau . \sigma
        \and
        \Gamma \proves e : \tau
    } { \Gamma \proves f ~ e : \sigma [ e / x ] }
    ...
    \inferrule*[right=Pi]{
        \Gamma \proves \tau : r
        \and
        \Gamma, x : \tau \proves \sigma : s
    } { \Gamma \proves \PiT x : \tau . \sigma : t }
    \twhere (r,s,t) \in \curlys{(\ty, \ty, \ty), (\ty, \kind, \kind)}
...
The answer is we sort of can't actually \textbf{do} anything yet---much like the simply-typed $\lambda$-calculus, there actually aren't any well-typed ground terms.

However, using an appropriate context, we can still do some interesting things.
For the the next section we'll be using (and expanding) onto the following context:
\newcommand{\consT}{\texttt{cons}}
\newcommand{\nilT}{\texttt{nil}}
\newcommand{\vecT}{\texttt{Vector}}
\newcommand{\snat}{\mathcal{s}}
\eq{
    \N &: \ty \\
    0 &: \N \\
    \snat &: \PiT n : \N . \N \\
    \vecT &: \PiT n : \N . \ty \\
    \nilT &: \vecT ~ 0 \\
    \consT &: \PiT n : \N . \PiT x : \N . \PiT xs : \vecT ~ n . \vecT ~ (\snat~n)
}
...
\eq{
    \snat &: \N \to \N \\
    \vecT &: \N \to \ty \\
    \consT &: \PiT n : \N . \N \to \vecT ~ n \to \vecT ~ (\snat~n)
}
...
\begin{mathpar}
    \inferrule*[right=Axiom]{ }{ \Gamma \proves r : s }
    \tif (r,s) \in \mathcal{A}
